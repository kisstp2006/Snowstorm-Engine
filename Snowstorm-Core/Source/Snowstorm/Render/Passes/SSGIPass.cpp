...
namespace Snowstorm
{
	namespace
	{
		// Coarse march + refine budget. Half of SSR's 64/5 because SSGI runs RayCount rays per pixel instead of
		// one: at the default 4 rays this is already ~4x SSR's per-pixel march work, offset by the half-res grid.
		constexpr uint32_t kMaxSteps = 32;
		constexpr uint32_t kRefineSteps = 4;
		...
		// Mirrors SSGICB in SSGI.comp.hlsl field-for-field (std140/cbuffer 16-byte rows). Keep in lockstep.
		struct SSGICB
		{
		...
		// Binding indices in SSGI.comp.hlsl set 0.
		constexpr uint32_t kNormalBinding = 0;
	...
	void SSGIPass::EnsureResources()
	{
		...
		Ref<Shader> cs = Application::Get().GetServiceManager().GetService<ShaderLibrary>().Load("Engine/Shaders/SSGI.comp.hlsl");
		SS_CORE_ASSERT(cs, "Failed to load SSGI compute shader");
		if (!cs->IsReady())
		{
		...
		p.Shader = cs;
		p.DebugName = "SSGIPipeline";
		m_Pipeline = Pipeline::Create(p);
		SS_CORE_ASSERT(m_Pipeline, "Failed to create SSGI pipeline");
		...
		{
			m_ParamBuffers[i] = Buffer::Create(sizeof(SSGICB), BufferUsage::Uniform, nullptr, true, "SSGICB");
		}
	}

	void SSGIPass::Dispatch(const Ref<CommandContext>& ctx, const uint32_t frameIndex, const glm::mat4& viewProj,
	                        const glm::vec3& camPos, const float giRange, const float nearPlane, const float farPlane,
	                        const float giIntensity, const float iblIntensity, const uint32_t rayCount,
	                        const uint32_t frameCounter, const uint32_t prefilteredCubeIndex,
	                        const Ref<TextureView>& normal, const Ref<TextureView>& depth,
	                        const Ref<TextureView>& prevColor, const Ref<TextureView>& velocity,
	                        const Ref<TextureView>& output, const uint32_t outW, const uint32_t outH)
	{
		if (!ctx || !normal || !depth || !prevColor || !velocity || !output || outW == 0 || outH == 0)
		{
			return;
		}
		...
		cb.Thickness = (giRange / static_cast<float>(kMaxSteps)) * kThicknessScale;
		cb.PrefilteredCubeIndex = prefilteredCubeIndex;
		cb.IBLIntensity = iblIntensity;
		m_ParamBuffers[frameIndex]->SetData(&cb, sizeof(SSGICB), 0);

		const auto& layouts = m_Pipeline->GetSetLayouts();
		SS_CORE_ASSERT(!layouts.empty() && layouts[0], "SSGI pipeline missing set=0 layout");
		if (!m_Sets[frameIndex])
		{
			DescriptorSetDesc dsd{};
			dsd.DebugName = "SSGISet";
			m_Sets[frameIndex] = DescriptorSet::Create(layouts[0], dsd);
		}
		m_Sets[frameIndex]->SetTexture(kNormalBinding, normal);
		...
		const BufferBinding cbBB{.Buffer = m_ParamBuffers[frameIndex], .Offset = 0, .Range = sizeof(SSGICB)};
